...
async def list_servers(active_only: bool = False) -> list[Server]:
    ...
    async with get_db() as db:
        if active_only:
            cursor = await db.execute(
                """SELECT id, friendly_name, url, active, autoload, model_path, parallel, ctx_size, created_at
...
async def create_server(
    ...
    async with get_db() as db:
        ...
        now = datetime.utcnow()
        cursor = await db.execute(
            """INSERT INTO servers (friendly_name, url, active, autoload, model_path, parallel, ctx_size, created_at)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
            (friendly_name, url, 1 if active else 0, 1 if autoload else 0, model_path, parallel, ctx_size, now)
        )
        await db.commit()
        return Server(
            id=cursor.lastrowid,
...
async def update_server(server_id: int, updates: dict[str, Any]) -> Server | None:
    ...
    async with get_db() as db:
        set_clauses = []
        values = []
        valid_keys = ("friendly_name", "url", "active", "autoload", "model_path", "parallel", "ctx_size")
        for key, value in updates.items():
            if key not in valid_keys:
                continue
            if value is not None:
                if key in ("active", "autoload"):
                    set_clauses.append(f"{key} = ?")
